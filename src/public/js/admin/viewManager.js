...
const getPermission = async (managerUsername) => {
    const res = await axios({
        method: 'POST',
        url: 'https://covid-19-management-sys-19tn.herokuapp.com//api/v1/manager/permission/get',
        data: {
            managerUsername,
        },
    });
...
const updatePermission = async (managerUsername, changeData) => {
    const res = await axios({
        method: 'POST',
        url: 'https://covid-19-management-sys-19tn.herokuapp.com//api/v1/manager/permission/update',
        data: {
            managerUsername,
...
const showHistory = async (managerUsername) => {
    const res = await axios({
        method: 'POST',
        url: 'https://covid-19-management-sys-19tn.herokuapp.com//api/v1/manager/log',
        data: {
            managerUsername,
        },
    });
...
const updateState = async (state) => {
    const res = await axios({
        method: 'POST',
        url: 'https://covid-19-management-sys-19tn.herokuapp.com//api/v1/manager/update',
        data: {
            managerUsername,
            role: `${state}_manager`,
        },
...
const getCheckedElement = (username) => {
    ...
    checkbox.forEach((element) => {
        if (element.checked == true && element.value !== 'all') permissions.push(element.value);
    });
    return permissions;
};
